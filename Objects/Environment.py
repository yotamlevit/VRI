...
from Movement.Motor import Motor
from Objects.Robot import Robot
from Objects import Robot as rob
from Objects import Wall as wal
...
class Environment:
    ...
    def environment_from_file(file_name='environment.xml'):
        tree = ET.parse(Environment.data_path(file_name))
        root = tree.getroot()
        height = (False, None)
        width = (False, None)
        robot = (False, None)
        ...
        for child in root:
            ...
            if tag == 'height':
                ...
                temp_r = Robot.robot_from_file(child)
                if temp_r[0]:
                    robot = (True, temp_r[1])
                else:
                    print(temp_r)
                ...
                objects = Environment.handle_objects_from_file(child)
    ...
    def handle_objects_from_file(root):
        ...
        for child in root:
            tag = child.tag.lower()
            if tag == 'wall':
                temp_w = Wall.wall_from_file(child)
                if temp_w[0]:
                    bool_obj = True
